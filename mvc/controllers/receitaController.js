...
module.exports = (app) => {
    ...
    app.get("/receita/larica", async (req, res) => {
        res.setHeader("Access-Control-Allow-Origin", "*");
        const receitaDAO = new CardapioDAO();
        const receitasLarica = await receitaDAO.consultarReceitasPorCategoria('Receitas da larica');
        res.render("larica", { receitas: receitasLarica });
    });
    app.get("/receita/fit", async (req, res) => {
        res.setHeader("Access-Control-Allow-Origin", "*");
        const cardapioDAO = new CardapioDAO();
        const receitasFit = await cardapioDAO.consultarReceitasPorCategoria('Receitas fit');
        res.render("fit", { receitas: receitasFit });
    });
    app.get("/receita/vo", async (req, res) => {
        res.setHeader("Access-Control-Allow-Origin", "*");
        const cardapioDAO = new CardapioDAO();
        const receitasFit = await cardapioDAO.consultarReceitasPorCategoria('Receitas de vó');
        res.render("vo", { receitas: receitasFit });
    });
